...
export default function ImportarPage() {
  ...
  async function preValidarLote() {
    ...
    if (!cnpj) return alert("CNPJ da empresa não foi detectado. Selecione os arquivos XML novamente.");
    setRunning(true);
    setErroGeral("");
    setResumo(null);
        ...
        <main className="flex-1 p-6">
          <div className="mb-6 bg-emerald-50 border-l-4 border-emerald-500 rounded p-4">
            ...
            <p className="text-sm text-slate-700">
              ...
              Payload ~20x menor. Suporta <b>milhares de notas</b> na Vercel sem estourar os 4.5 MB.
            </p>
          </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {files.length > 0 && (
                    <div className="text-xs text-slate-600 mt-1 space-y-0.5">
                      <div>{files.length} XMLs · {totalMB} MB no disco</div>
                ...
                {detectando && (
                  <div className="text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 rounded px-3 py-2">
                    Detectando empresa automaticamente a partir dos XMLs...
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mt-1">
                    NÃO é o faturamento do mês nem do lote. É o total dos <b>últimos 12 meses</b> da empresa.
                    Se deixar vazio, o sistema estima (lote x 12).
                  </p>
                </div>
                <button
                ...
                {files.length > 0 && (
                  ...
                  >
                    {zipando ? "Gerando..." : `Baixar .zip com TODOS os ${files.length} XMLs (pra arrastar no Claude Pro)`}
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="mt-6 space-y-4">
                  ...
                  {Object.entries(preValidacao.por_status).map(([cs, qtd]) => {
                    ...
                    return (
                      <div key={cs} className={`px-3 py-1.5 rounded ${ok ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800"}`}>
                        <b>cStat {cs}:</b> {qtd} nota(s) {ok ? "OK" : "excluída(s)"}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-gradient-to-br from-orange-500 to-orange-700 rounded-lg p-5 shadow-lg">
                ...
                <div className="mt-3 pt-3 border-t border-orange-400">
                  ...
                  >
                    {zipando ? "Gerando .zip..." : `BAIXAR .ZIP COM TODOS OS ${files.length} XMLs (para anexar no Claude)`}
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-5">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  <a href="https://grok.com/" target="_blank" rel="noopener" className="px-4 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-md text-sm">Grok</a>
                ...
                <details>
                  <summary className="text-xs text-slate-500 cursor-pointer">Ver o dossiê completo (o que vai para a IA)</summary>
          ...
          {resumo && !erroGeral && (
            <div className="mt-6 bg-emerald-50 border border-emerald-200 rounded-lg p-5">
              ...
              <ul className="text-sm text-slate-700 space-y-1">
                <li>Notas ÚNICAS AUTORIZADAS processadas: <b>{resumo.processadas.toLocaleString("pt-BR")}</b></li>
                {resumo.dedup_canceladas > 0 && (
                  <li className="text-orange-700 bg-orange-50 px-2 py-1 rounded">
                    <b>{resumo.dedup_canceladas.toLocaleString("pt-BR")}</b> NF-e canceladas/denegadas (cStat diferente de 100/150) — EXCLUÍDAS do faturamento e da apuração de impostos. Isso é o comportamento correto conforme SEFAZ.
                  </li>
                )}
                {(resumo.dedup_no_lote > 0 || resumo.dedup_no_banco > 0) && (
                  <li className="text-amber-700 bg-amber-50 px-2 py-1 rounded">
                    <b>Deduplicação:</b> de {resumo.dedup_recebidas.toLocaleString("pt-BR")} XMLs recebidos,
                ...
                {resumo.parseErros.length > 0 && (
                  <li>{resumo.parseErros.length} XML(s) com erro de parse (ignorados)</li>
                )}
              </ul>
