...
export default function EstagioPage() {
  ...
  async function cadastrar() {
    ...
    if (!cpf || !nomeCompleto || !dataAdmissao || !valorBolsa) {
      setErroCadastro("Preencha CPF, nome, data de admissão e valor da bolsa.");
      return;
    }
    setSalvando(true);
    try {
      const res = await fetch("/api/dp/estagio-novo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cpf, nomeCompleto, cargo, dataAdmissao, valorBolsa: Number(valorBolsa) }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Erro ao cadastrar.");
      setMensagemCadastro("Estagiário cadastrado! Sem INSS, IRRF ou FGTS (bolsa-auxílio).");
      setCpf("");
      setNomeCompleto("");
      setCargo("");
      setDataAdmissao("");
  ...
  async function processarPagamento() {
    ...
    if (!colaboradorId) {
      setErroPagamento("Selecione um estagiário.");
      return;
    }
    setProcessando(true);
    try {
      const res = await fetch("/api/dp/folha/processar-estagiario", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ colaboradorId: Number(colaboradorId), competencia }),
      });
      const data = await res.json();
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Estágio</h1>
          <p className="text-sm text-slate-500">Bolsa-auxílio, sem vínculo empregatício (Lei 11.788/2008)</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        <h2 className="text-sm font-semibold text-slate-900">Novo Estagiário</h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-slate-500 block mb-1">Cargo/Área</label>
            <input
              type="text"
              className="w-full border border-slate-200 rounded-lg p-2.5 text-sm"
              value={cargo}
        ...
        <div>
          <label className="text-sm text-slate-500 block mb-1">Valor da bolsa mensal (R$)</label>
          <input
            type="number"
            min={0}
            step="0.01"
        ...
        >
          ...
          {salvando ? "Salvando..." : "Cadastrar Estagiário"}
        </button>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        <h2 className="text-sm font-semibold text-slate-900">Processar Pagamento de Bolsa</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-slate-500 block mb-1">Estagiário</label>
            <select
              className="w-full border border-slate-200 rounded-lg p-2.5 text-sm"
              value={colaboradorId}
            ...
            >
              ...
              {estagiarios.map((e) => (
                <option key={e.colaborador_id} value={e.colaborador_id}>
                  {e.nome_completo} — R$ {Number(e.salario_base).toFixed(2)}
                </option>
              ))}
            </select>
          </div>
        ...
        {resultadoPagamento && (
          <div className="mt-3 p-3 bg-slate-50 rounded-lg text-sm">
            ...
            <p className="text-slate-500 text-xs mt-1">Sem INSS, IRRF ou FGTS — bolsa cheia.</p>
          </div>
        )}
      </div>
    </div>
