...
import { db } from "@/db";
import { contasPagar, contasBancarias } from "@/db/schema-financeiro";
import { eq, and } from "drizzle-orm";

export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withAuth(async (ctx) => {
    const { id } = await params;
    const [conta] = await db.select().from(contasPagar).where(
      and(eq(contasPagar.id, Number(id)), eq(contasPagar.empresa_id, ctx.empresaId))
    );
    if (!conta) {
