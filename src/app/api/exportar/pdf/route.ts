...
function tabelaMini(doc: any, headers: string[], rows: string[][], widths: number[]) {
  ...
  const headerH = 15;
  const rowH = 12;
  const maxY = doc.page.height - doc.page.margins.bottom - 24; // reserva pro rodapé
  ...
  if (rows.length > rowsCabem) {
    doc.fillColor(C.gray).font("Helvetica-Oblique").fontSize(6.5)
      .text(`(+ ${rows.length - rowsCabem} linha(s) omitidas — ver Excel completo)`, startX, y + 1, { width: totalW, lineBreak: false });
    y += 9;
  }
  doc.y = y + 3;
  doc.x = startX;
...
export async function GET() {
  ...
  // =========================================================
  // PÁGINA 1 — Capa + Resumo + Balanço + Apuração + Balancete
  // =========================================================
  ...
  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(16).text("PARECER CONTÁBIL-FISCAL", 40, 46);
  doc.fillColor(C.gold).font("Helvetica").fontSize(9).text(`Escrituração · Apuração · Auditoria — ${new Date().toLocaleDateString("pt-BR")}`, 40, 66);

  doc.strokeColor(C.gold).lineWidth(1).moveTo(40, 82).lineTo(40 + W, 82).stroke();

  doc.fillColor(C.slate).font("Helvetica-Bold").fontSize(9).text("CLIENTE", 40, 89);
  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(11).text(emp.nome, 40, 100);
  doc.fillColor(C.gray).font("Helvetica").fontSize(8.5)
    .text(`CNPJ ${emp.cnpj}   ·   Regime ${emp.regime.replace("_", " ")}   ·   ${emp.segmento ?? "COMERCIO"}`, 40, 114);
  ...
  // Balanço
  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(9.5).text("BALANÇO PATRIMONIAL", 40, doc.y);
  doc.y += 13;
  tabelaMini(
    doc,
    ["Grupo", "Saldo"],
  ...
  // Apuração
  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(9.5).text("APURAÇÃO DE IMPOSTOS", 40, doc.y + 3);
  doc.y += 13;
  const apRowsFmt = apRows.slice(0, 8).map((r) => [
    `${r.periodo} · ${r.imposto}`,
    fmtMoeda(r.debito),
    fmtMoeda(r.credito),
    fmtMoeda(r.a_pagar),
  ...
  // Balancete (na mesma página 1)
  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(9.5).text("BALANCETE DE VERIFICAÇÃO", 40, doc.y);
  doc.y += 13;
  ...
  // =========================================================
  // PÁGINA 2 — Auditoria + Análise + DRE + Assinatura
  // =========================================================
  addPageWithFooter();

  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(13).text("AUDITORIA R08 — MONOFÁSICO PIS/COFINS", 40, 26);
  doc.fillColor(C.gray).font("Helvetica").fontSize(7.5)
    .text("Lei 10.147/2000 · Lei 10.485/2002 · Lei 13.097/2015", 40, 43);
  doc.strokeColor(C.gold).lineWidth(0.5).moveTo(40, 54).lineTo(40 + W, 54).stroke();
  ...
  doc.roundedRect(40, 62, W, 36, 3).fillAndStroke(C.bg, statusCor);
  doc.fillColor(statusCor).font("Helvetica-Bold").fontSize(12).text(statusTxt, 50, 71, { width: W - 20 });
  doc.fillColor(C.slate).font("Helvetica").fontSize(8.5)
    .text(`Foram auditados ${resumo.qtd_notas} documentos fiscais quanto à classificação tributária monofásica.`, 50, 86, { width: W - 20 });
  ...
  if (audit.length > 0) {
    ...
    if (audit.length > 10) {
      doc.fillColor(C.gray).font("Helvetica-Oblique").fontSize(7.5)
        .text(`... e mais ${audit.length - 10} ocorrências. Ver Excel para lista completa.`, 40, doc.y);
    }
    doc.fillColor(C.red).font("Helvetica-Bold").fontSize(9.5)
      .text(`Crédito recuperável total: ${fmtMoeda(totalCredR08)}`, 40, doc.y + 2, { width: W });
    doc.y += 14;
  }

  doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(10).text("ANÁLISE E CONCLUSÕES", 40, doc.y);
  ...
  // DRE (na mesma página 2)
  doc.moveDown(0.2);
  for (const ex of dreExercicios) {
    doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(9.5).text(`DRE — EXERCÍCIO ${ex.ano}`, 40, doc.y + 2);
    doc.y += 13;
  ...
  doc.fillColor(C.gray).font("Helvetica-Oblique").fontSize(6.5)
    .text(
      "Nota metodológica: faturamento = SUM(vNF) das notas SAÍDA/VENDA/SERVIÇO com cStat=100 (autorizadas) e chaves únicas. Base para DAS Simples aplicada conforme LC 123/2006 Anexo I. Auditoria R08 conforme Leis 10.147/2000 e 10.485/2002. Este parecer é auxiliar; não substitui transmissão de SPED, PGDAS-D, DEFIS ou ECF pela contabilidade responsável. Para o razão completo e notas individuais, consulte o Excel exportado (10 abas).",
      40,
      doc.y,
