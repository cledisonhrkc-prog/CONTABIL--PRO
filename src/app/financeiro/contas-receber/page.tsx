...
import { ArrowLeft, Search, ArrowDownCircle, CheckCircle2 } from "lucide-react";
...
export default function ContasReceberPage() {
  ...
  async function load() {
    ...
    try {
      ...
      console.error(e);
      setErro(e.message || "Erro ao carregar contas a receber.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function baixar(id: number) {
    setBaixandoId(id);
    try {
      const res = await fetch(`/api/financeiro/contas-receber/${id}/baixar`, { method: "POST" });
      if (!res.ok) {
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-6 rounded-b-3xl shadow-lg mb-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <h1 className="text-white text-2xl font-bold mt-0.5">Contas a Receber</h1>
            <p className="text-indigo-100 text-sm mt-1">
              Títulos de clientes — R$ {totalEmAberto.toFixed(2)} em aberto
            </p>
          </div>
          <Link
            href="/financeiro"
      ...
      <div className="max-w-7xl mx-auto px-6 pb-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
            <ArrowDownCircle className="h-4 w-4 text-emerald-500" /> Contas
          </h2>
